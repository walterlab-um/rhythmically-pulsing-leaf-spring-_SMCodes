...
%% Analyze image
if any(d(:))  ; %for the case of non zero raw image - so there is something to look for
    
    d = medfilt2(d,[3,3]);
    
    % apply threshold
    if isa(d,'uint8') %determines the input type, we should have 16-bit
        d=d.*uint8(d>thres); %should leave original (slightly smoothed)
        %pixel contents where signal is larger than threshold and zeros
        %where signal is below threshold
    else
        d=d.*uint16(d>thres);
    end
